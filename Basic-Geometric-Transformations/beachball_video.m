...
pos_x = 300;
%550-floor(abs(exp(-i/100)*300*sin((2*pi*i/100)-90)) gia i=0
pos_y = 282;
...
for i = 1:280
    ...
    %dior8wsh megethous
 	rot_ball = imcrop(rot_ball,[(rbm-size(new_ball,1))/2 ...
        (rbn-size(new_ball,2))/2 120 120]);
    rot_mask = imcrop(rot_mask,[(rmm-size(new_mask,1))/2 ...
        (rmn-size(new_mask,2))/2 120 120]);

    %ypologizoume tis diastaseis - diaforetikes se kathe rotation
    padded_ball = padarray(rot_ball, [pos_y pos_x], 0, 'pre');
    padded_ball = padarray(padded_ball, [M-size(padded_ball,1) ...
        N-size(padded_ball,2)], 0, 'post');

    padded_mask = padarray(rot_mask, [pos_y pos_x],255,'pre');
    padded_mask = padarray(padded_mask,[M-size(padded_mask,1)...
        N-size(padded_mask,2)], 255, 'post');
    ...
    %aukshsh ths gwnias - afairoume gia na kanei rotate h mpala opws sto
    %video
    dg = dg - .05;
    
    %metatopish 3 pixels ana kyklo epanalhpshs ston aksona x
    pos_x = pos_x + 3;
    
    %f8inoysa metatopish kata ton aksona y
    pos_y = 550 - floor(abs(exp(-i/100)*300*sin((2*pi*i/100)-90)));
